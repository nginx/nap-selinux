...
# Disable SELinux booleans that may conditionally allow access
echo "[INFO] Disabling SELinux booleans for controlled policy behavior..."
setsebool -P httpd_builtin_scripting off
setsebool -P httpd_unified off
setsebool -P httpd_enable_cgi off
echo "[INFO] Booleans disabled: httpd_builtin_scripting, httpd_unified, httpd_enable_cgi"

# Check if all required SELinux booleans are OFF
if getsebool httpd_builtin_scripting httpd_unified httpd_enable_cgi | grep -vq 'off$'; then
    echo "✗ ERROR: One or more SELinux booleans are still ON"
    getsebool httpd_builtin_scripting httpd_unified httpd_enable_cgi
    exit 1
else
    echo "✓ All required SELinux booleans are OFF"
fi
